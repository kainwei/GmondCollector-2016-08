...
public class HBaseRegionVal {
  ...
  public HBaseRegionVal(String address, MetricConf mc){
    this.ip = address;
    this.mc = mc;
  }
  public String getIP(){
  ...
  public boolean writeHostOrigToHBase(HTable normalTable, 
                                      HTable nodispTable, 
                                      HTable alertTable) throws IOException {
    ...
    for (Entry<Long, MetricValue> e : metricValuesMap.entrySet()) {
      ...
      if (MetricDescriptor.isMetricAlertable(id)){
  ...
  public boolean writeHostHourToHBase(HTable ht) throws IOException{
    if (ht == null)
      return false;
