...
public class Summarizer extends TimerTask {


  static class MyHTableFactory extends HTableFactory {

    private volatile static  MyHTableFactory hbaseclient;
    @Override
    ...
    private MyHTableFactory(){
      super();
    }
    public static MyHTableFactory getHc(){
      if(hbaseclient == null) {
        synchronized (MyHTableFactory.class) {
          if(hbaseclient == null ) {
            hbaseclient = new MyHTableFactory();
          }
        }
      }
      return hbaseclient;
  ...
  public static final Logger LOG = Logger.getLogger(Summarizer.class.getName());
  ...
  public Summarizer(Configuration c) throws IOException {
    this.conf = c;
    htpConcurrency = c.getInt(BigMonitorConstants.HTableConcurrencyKey,
            BigMonitorConstants.HTableConcurrencyDefault);
    //htp = new HTablePool(c, htpConcurrency, MyHTableFactory.getHc());
    final HConnection conn1 = HConnectionManager.createConnection(conf);
    //htp = getHConnection(c);
    Date now = new Date();
    SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
    htp = conn1;
    System.out.println("Debug Log(boyan) init htp  in : " + dateFormat.format(now) );
      ...
      for (final Long metricID : mc.getImportantIDs()) {
        Future<String> r = executor.submit(new Callable<String>(){
          ...
          public String call() {
            ...
            Scan scan = new Scan(Bytes.toBytes(host + "#" + metricID + "#"
                    + timeFrom), Bytes.toBytes(host + "#" + metricID + "#" + timeTo));
            //add log
            String start=host + "#" + metricID + "#" + timeFrom;
            String stop=host + "#" + metricID + "#" + timeTo;
            //LOG.info("amina haha"+start);
            //LOG.info("amina haha"+stop);

            MetricValue mv = new MetricValue(metricID,102400);
            ResultScanner rs = null;
            ...
            try {
              ...
              for (Result r : rs) {
                ...
                // System.out.println("aminamiao" +val);
                mv.addValue(val);
              }
              ...
              t1.close();
              if (mv.getSize()== 0) {
                //LOG.warn("amina Data not found: " + host + "#" + metricID + "#" + timeTo);//TODO
                return null;
              }
              byte[] aveBytes = Bytes.toBytes(mv.getAverageString());
              Put p = new Put(Bytes.toBytes(host + "#" + metricID + "#" + timeTo));
...
//              LOG.warn(Bytes.toString(p.getRow()) + " " + Bytes.toString(aveBytes));//TODO
  ...
  public static void main(String argv[]) throws IOException {
    ...
    Summarizer worker = new Summarizer(conf);
...
//    worker.run();
  ...
  public static void printUsage(){
    System.err.println("java " + Summarizer.class.getName() + " -start");
  }
