...
public class HttpServer  {
  public static final Log LOG = LogFactory.getLog(org.apache.hadoop.http.HttpServer.class);
  ...
  public HttpServer(String name, String bindAddress, int port,
    ...
    //String dir = getClass().getClassLoader().getResource(".").toString();
   // System.out.println(dir);
    webAppContext.setResourceBase("./GmondCollector_hbase1/webapps/");
    webAppContext.setContextPath("/");


        //webAppContext.setWar(appDir + "/" + name);
    //webAppContext.setWelcomeFiles(new String[]{"main.jsp"});
  ...
  public void addFilter(String name, String classname,
    ...
    for (Map.Entry<Context, Boolean> e : defaultContexts.entrySet()) {
      if (e.getValue()) {
        ...
        System.out.println("Added filter " + name + " (class=" + classname
  ...
  public void addGlobalFilter(String name, String classname,
    ...
    System.out.println("Added global filter" + name + " (class=" + classname + ")");
  }
   ...
   * @throws java.io.IOException if 'webapps' directory cannot be found on CLASSPATH.
   */
    ...
    try {
      ...
      while (true) {
        try {
          ...
          System.out.println("Port returned by webServer.getConnectors()[0]." +
                "getLocalPort() before open() is "+ port + 
                ". Opening the listener on " + oriPort);
          listener.open();
          port = listener.getLocalPort();
          System.out.println("listener.getLocalPort() returned " + listener.getLocalPort() + 
          ...
          if (port < 0) {
            ...
            int numRetries = 1;
            while (port < 0) {
              System.out.println("listener.getLocalPort returned " + port);
              if (numRetries++ > MAX_RETRIES) {
                throw new Exception(" listener.getLocalPort is returning " +
                        "less than 0 even after " +numRetries+" resets");
              }
              for (int i = 0; i < 2; i++) {
                System.out.println("Retrying listener.getLocalPort()");
                port = listener.getLocalPort();
                if (port > 0) {
                  break;
                }
              ...
              System.out.println("Bouncing the listener");
              listener.close();
              Thread.sleep(1000);
          ...
          System.out.println("Jetty bound to port " + port);
          webServer.start();
          break;
          ...
          if (ex instanceof BindException) {
            ...
            System.out.println("HttpServer.start() threw a non Bind IOException"); 
            throw ex;
          }
        } catch (MultiException ex) {
          System.out.println("HttpServer.start() threw a MultiException"); 
          throw ex;
        }
